...
from config import EngineConfig, ASRConfig, ASREngine
...
class FasterWhisperASR:
    ...
    async def load(self) -> None:
        ...
        self._model = await asyncio.to_thread(
            WhisperModel, self.cfg.asr_model, device=self.cfg.asr_device, compute_type=self.cfg.asr_compute
        )
        log.info("ASR: faster-whisper (%s, %s/%s) загружен", self.cfg.asr_model, self.cfg.asr_device, self.cfg.asr_compute)
